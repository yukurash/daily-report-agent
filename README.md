...
- 「Training 始めた」と言うだけで時間計測スタート
- 「次は 1on1」で前タスクを自動停止して切替
- 「Onboarding をまた再開」で同じタスクを再開(時間が累積)
- 「終わり、日報出して」で OneNote 貼付用の **PNG / HTML** を一発出力

```
ユーザー(日本語) → 日報アシスタント Agent → Python CLI → outputs/<日付>/
```
...
エージェントが裏で CLI を呼び出し、1〜2 行で結果を返します。
...
|---|---|
| 「Training 始めた」 | タスク開始 |
| 「次は 1on1 w/田中さん」 | 前を停止 → 1on1 を開始 |
| 「いま何してる」 | 現在のタスクと一覧を 3 行で要約 |
| 「Onboarding をまた再開」 | 同名タスクで `start`(時間が累積) |
| 「メール仕分けを 30 分でつけて」 | 計測なしで 0.5h を手動加算 |
| 「No 5 を 1.5 時間に直して」 | 時間を上書き |
| 「No 5 の名前を MyHorizon に」 | リネーム |
| 「終わり、日報出して」 | PNG / HTML を生成 |
